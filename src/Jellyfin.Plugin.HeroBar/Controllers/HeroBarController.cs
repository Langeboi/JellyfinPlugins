...
namespace Jellyfin.Plugin.HeroBar.Controllers
{
    ...
    {
        // Embedded files cannot change while the plugin is loaded, so each one
        // is read and hashed once. Only files that exist are kept - caching a
        // miss for whatever name is asked for would let random URLs grow this
        // without limit.
